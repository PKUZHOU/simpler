...
def main():
    ...
    parser.add_argument("--nranks", type=int, default=8,
                        help="Number of ranks / devices (default: 8)")
    parser.add_argument("--root", type=int, default=0,
                        help="Root rank for collective operations (default: 0)")
    parser.add_argument("--build-dir", default=None,
                        help="Build cache directory")
    parser.add_argument("--artifact-dir", default=None,
                        help="Artifact output directory")
    parser.add_argument("--orch-func", default=None,
                        help="Orchestration function name (default: from kernel_config)")
    parser.add_argument("--skip-compile", action="store_true",
                        help="Skip compilation (use existing artifacts)")
    ...
    # Source CANN env if not already set
    if not os.environ.get("ASCEND_HOME_PATH"):
        cann_env = "/usr/local/Ascend/cann-8.5.0/set_env.sh"
        if os.path.exists(cann_env):
            logging.info(f"Sourcing {cann_env}")
            os.system(f"source {cann_env}")
