...
void aicpu_orchestration_entry(PTO2Runtime* rt, uint64_t* args, int arg_count,
    ...
    uint64_t sdma_workspace = pto2_rt_get_sdma_workspace(rt);

    LOG_INFO(rt, "async_demo: SIZE=%d, event_handle_output=0x%lx, sdma_workspace=0x%lx",
    ...
    if (sdma_workspace != 0) {
        ...
        LOG_INFO(rt, "async_demo: HW mode — submitted TPUT_ASYNC producer (func_id=2)");
    } else {
        ...
        LOG_INFO(rt, "async_demo: Sim mode — submitted simulated producer (func_id=0)");
    }
