 ...
 * distributed_worker (HCCL window addresses, device context pointer).
 * Creates a single AIV task with func_id=0 (treduce kernel).
 *
 * args layout:
 *   args[0] = input device pointer  (in HCCL RDMA window)
 *   args[1] = output device pointer (regular device memory)
 *   args[2] = nranks
 *   args[3] = root rank
 *   args[4] = HcclDeviceContext device pointer
 */
